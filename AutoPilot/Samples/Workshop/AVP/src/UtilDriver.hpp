...
public:
	...
	* @param
	*   gear: Gear to be set, defaulted with EGearMode_Drive
	* @param
	...
	*/
	static void SetControl(const long long timestamp, const double throttle, const double brake,
						const double steering, const EGearMode gear = EGearMode::EGearMode_Drive,
						const int mainVehicleId = 0)
	{
		...
		pControl->gear = gear;
		SimOneSM::SetDrive(mainVehicleId, pControl.get());
	}
