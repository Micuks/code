...
\begin{frame}[fragile]{数据基本信息}
	\only<1>{
		波士顿房价数据集(Boston Housing Dataset)是机器学习中非常经典的数据集, 来自1970年的美国人口普查数据, 主要用于研究房屋价格和各种因素之间的关系. 其详细信息如下.
	}


	\begin{block}{特征}<only@2>
		\begin{itemize}
			\item \textbf{CRIM} 城镇的人均犯罪率;
			\item \textbf{ZN} 大于25,000平方英尺的住宅用地比例;
			\item \textbf{INDUS} 非零售商业用地的比例(亩);
			\item \textbf{CHAS} 查尔斯河虚拟变量(如果地块与河流接壤为1, 否则为0);
			\item \textbf{NOX} 一氧化氮浓度(每千万分之一);
			\item \textbf{RM} 每个住宅的平均房间数;
			\item \textbf{AGE} 1940年以前建造的自住单位的比例;
			\item \textbf{DIS} 到波士顿五个就业中心的加权距离;
			\item \textbf{RAD} 高速公路通达性的指数;
			\item \textbf{TAX} 每10,000美元的全额物业税率;
			\item \textbf{PTRATIO} 城镇的学生与教师比例;
			\item \textbf{B} $1000(B_k-0.63)^2$, 其中$B_k$是城镇中黑人的比例;
			\item \textbf{LSTAT} 人口中地位较低的人的百分比;
		\end{itemize}
	\end{block}
	\begin{block}{目标变量}<only@3>
		\begin{itemize}
			\item \textbf{MEDV} 自住房的中位数价格(单位: \$1000美元). 即希望预测的房屋价格.
		\end{itemize}
	\end{block}
	\begin{block}{数据集特点和挑战}<only@3>
		\begin{itemize}
			\item \textbf{数据规模 }数据集通常包含506个观测点, 其中每个观测点都有13个特征和1个目标变量;
			\item \textbf{缺失值 } 波士顿房价数据集没有缺失值, 因此处理起来会相对方便;
			\item \textbf{异常值 } 需要检查数据中的异常值或者离群值, 因为他们会影响模型的性能;
			\item \textbf{特征工程 } 虽然数据集已经提供了13个特征, 但是可能需要进行特征选择或者特征工程来提供模型的性能.
		\end{itemize}
	\end{block}
\end{frame}
...
\section{红酒质量预测}
\begin{frame}[fragile]{红酒质量预测任务}
	酒质量预测任务旨在基于物理化学测试的结果来预测红酒和白酒的质量。在该任务中, 使用一系列机器学习模型进行预测，模型通过分析红酒或白酒的一系列化学属性来预测其质量。预测的结果通常为0-10的得分，代表酒的整体质量。
\end{frame}
\subsection{任务介绍}
\begin{frame}[fragile]{任务介绍}
	对该任务, 着重于如下方面.
...
\begin{frame}[fragile]{使用Optuna优化XGBoost的超参数选择}
	...
	\only<4>{
		\begin{figure}[htbp]
			\centering
			\caption{Optuna的优化参数选择效果}
			\label{fig:Optuna的优化参数选择效果}
			\includegraphics[width=0.95\textwidth]{figures/optuna.png}
		\end{figure}
	}
\end{frame}
...
\begin{frame}[fragile]{训练介绍}
	...
	\begin{block}{学习率规划器}<only@2>
		\mintinline[breaklines=true]{bash}|ReduceLROnPlateau|是一个学习率调度策略, 当模型的精确度停滞不前的时候, 会自动降低学习率:
		\begin{itemize}
...
\begin{frame}[fragile]{模型介绍}
	\begin{block}{特征提取: TfIdfVectorizer}<only@1>
		...
		\begin{itemize}
			\item \mintinline[breaklines=true]{bash}|stop_words='english'| 意味着在向量化过程中会排除英语中的常用停用词;
			\item \mintinline[breaklines=true]{bash}|max_df=0.7| 意味着当一个词出现在超过70\%的文档中的时候会被忽略, 这有助于排除太过常见的词汇, 例如``is'', ``the'', ``and''等. 虽然IDF已经为罕见词语提供较高值, 这样的整个数据集每个文档都出现的词语提供较低值, 直接去除也会更加方便;
		\end{itemize}
	\end{block}
	\begin{block}{分类模型: PassiveAggressiveClassifier}<only@2>
		...
		其参数\mintinline[breaklines=true]{bash}|max_iter=50| 表示模型的最大迭代次数为50.
	\end{block}
\end{frame}
\subsection{训练介绍}
