...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # ((N,H * H,H = N,H) + (N,D * D,H = N,H) + (N, H))
    next_h = np.tanh(prev_h.dot(Wh) + x.dot(Wx) + b.transpose()[np.newaxis, :])

    # store values in cache for backward prop
...
def rnn_step_backward(dnext_h, cache):
    ...
    # X refers to X in tanh(X)
    dX = dnext_h * (1 - next_h**2)  # (N, H) * (N, H) = (N, H)
    db = np.sum(dX * 1, axis=0)  # (H,)
    dprev_h = dX.dot(Wh.transpose())  # (N, H).dot(H, H) = (N, H)
    dWh = prev_h.transpose().dot(dX)  # (H, N).dot(N, H) = (H, H)
    dx = dX.dot(Wx.transpose())  # (N, H).dot(H, D) = (N, D)
    dWx = x.transpose().dot(dX)  # (D, N).dot(N, H) = (D, H)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # list to store step cache
    step_caches = []

    # extract dimensions from variable shapes
    N, T, D = x.shape
    H = h0.shape[1]
    # print(f'N={N},D={D},H={H},T={T}')

    h = np.zeros((T, N, H))
    # transpose x to simplify calculation
    x_T = x.transpose((1, 0, 2))

    # use initial hidden state to calculate h[0]
    h[0], step_cache = rnn_step_forward(x_T[0], h0, Wx, Wh, b)
    step_caches.append(step_cache)
    for i in range(1, T, 1):
        h[i], step_cache = rnn_step_forward(x_T[i], h[i-1], Wx, Wh, b)
        step_caches.append(step_cache)

    # restore the shape of h
    h = h.transpose((1, 0, 2))
    # store values needed in the backward pass in cache
    cache = (x, h0, Wx, Wh, b, h, step_caches)
...
def rnn_backward(dh, cache):
    ...
    x, h0, Wx, Wh, b, h, step_caches = cache
    N, T, H = dh.shape
    D = x.shape[2]

    # transpose dh to help calculation
    ...
    for t in range(T-1, -1, -1):
        dh_curr = dh_T[t] + dprev_hi
        dxi, dprev_hi, dWxi, dWhi, dbi = rnn_step_backward(
            dh_curr, step_caches[t])
        dx[t] += dxi
        dWx += dWxi
        dWh += dWhi
        db += dbi
