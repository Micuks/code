...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (1)Use an affine transformation to compute the initial hidden state from
        # CNN image features
        h0, proj_cache = affine_forward(
            features, W_proj, b_proj)  # (N,D).(D,H)=(N,H)

        # (2)Use a word embedding layer to transform the words in captions_in from indices
        # to vectors
        word_embedded, embed_cache = word_embedding_forward(
            captions_in, W_embed)  # (N,T) to (N,T,W)
        ...
        # compute loss using temporal softmax
        loss, dscores = temporal_softmax_loss(
            scores, captions_out, mask, verbose=False)  # dscores(N, T, V)

        # Start of backward pass
        dh, dW_vocab, db_vocab = temporal_affine_backward(
            dscores, scores_cache)  # dh(N, T, H), dW_vocab(H, M), db_vocab(M,)
        ...
        dW_embed = word_embedding_backward(
            dword_embedded, embed_cache)  # dW_embed(V, D)

        dh0, dW_proj, db_proj = affine_backward(
            dh0, proj_cache)  # dh0(N, D), dW(D, M), db(M,)
        # End of backward pass

        # save gradients to dict grads
    ...
    def sample(self, features, max_length=30):
        ...
        word_step = self._start * np.ones((N, 1), dtype=np.int32)
        # embed first word
        V, D = W_embed.shape
