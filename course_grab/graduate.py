...
ids = [["3311101694", "研究生英语国际学术交流8班"], ["3321101666",
                                         # [[id, 班级或上课时间], ...]
                                         "新时代中国特色社会主义理论与实践13班"]]
...
def get_class_page(header, serverid=0):
    ...
    soup = BeautifulSoup(req.text, 'lxml').find_all(
        'tr', onmouseout="SetRowBgColor(this,false)")
    return soup
...
def xuanke(html, id, serverid=0):
    ...
    class_url = f'http://10.3.255.3{
        serverid}/Lesson/PlanSelClass.aspx?EID={eid}'

    req = requests.get(class_url, headers=header)
    source = req.text

    __EVENTVALIDATION = re.findall(
        r'<input type="hidden" name="__EVENTVALIDATION" id="__EVENTVALIDATION" value="(.*?)" />', source)[0]
    __VIEWSTATE = re.findall(
        r'<input type="hidden" name="__VIEWSTATE" id="__VIEWSTATE" value="(.*?)"', source)[0]

    soup = BeautifulSoup(source, 'lxml').find_all(
        'tr', onmouseout="SetRowBgColor(this,false)")
...
if __name__ == "__main__":
    target_time = time.mktime(time.strptime(
        "2024-9-5 13:59:00", "%Y-%m-%d %H:%M:%S"))

    header = login("ASP.NET_SessionId=1d1b0xup3rglzylttmxnmork")  # 这里需要修改
    counter = 1
    while True:
        now = time.time()
        if now > target_time:
        ...
        for id in tmp_ids:
            for s in classes:
                ...
                if id[0] in html:
                    if '正在选课' in html:
                        threading.Thread(
                            target=xuanke, args=(html, id,)).start()
                        # xuanke(html)
                    else:
